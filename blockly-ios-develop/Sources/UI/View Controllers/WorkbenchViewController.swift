  ...
  */
  ...
  fileprivate func commonInit() {
   ...
   deinit {
    ...
    ///2017 08 21 在页面销毁的时候 清除函数
    FunctionControl.functionControl.names.removeAll()
      ...
      if touchTouchingTrashCan && blockLayout.block.deletable {
        ...
        do {
          ...
          var allBlocksToRemove = blockLayout.block.allBlocksForTree()
            ...
            ///／2017 08 16 删除函数定义时 删除保存的函数数据
            let block = allBlocksToRemove.first;
            if block!.name == "function" {
                let name = block?.getBlockValues()[0];
                if name != "" && name != nil {
                    
                    var names = FunctionControl.functionControl.names;
                    
                    names.removeValue(forKey: block!.uuid);
                    FunctionControl.functionControl.names = names;
...
extension WorkbenchViewController {
    ...
    open func getBlockTree() -> [Block] {
        
        let blocks = self.workspace?.allBlocks;
        
        var blockTrees: [Block] = Array();
        
        for blockDic in blocks! {
            
            let block: Block = blockDic.value;
            
            if ["while_start","while","function"].contains(block.name) {
                
                blockTrees.append(block);
