...
open class LayoutEngine: NSObject {
  ...
  public fileprivate(set) final var minimumScale: CGFloat = LayoutConfig().minScale

  /// The maximum scale that the engine can have, relative to the Workspace coordinate system.
  public fileprivate(set) final var maximumScale: CGFloat = LayoutConfig().maxScale
  ...
  public final var scale: CGFloat = LayoutConfig().defaultScale {
    didSet {
   ...
   */
  public init(
    config: LayoutConfig = LayoutConfig(), rtl: Bool? = nil, minimumScale: CGFloat = LayoutConfig().minScale,
    maximumScale: CGFloat = LayoutConfig().maxScale) {
    self.config = config
    self.rtl =
