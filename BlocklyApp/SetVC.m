...
- (void)viewDidLoad {
    ...
    self.dataSource = [NSMutableArray arrayWithArray:@[@"头像",@"昵称",@"性别",@"手机号",@"程序延迟时间(ms)",@"判断程序延迟时间(ms)",@"红外线参数配置",@"声控参数配置",@"超声波参数控制"]];
...
- (void)saveChangeWidth:(int)index {
    ...
    NSMutableDictionary * intDic = [NSMutableDictionary dictionary];
...
//            [popPC setSourceRect:CGRectMake(0, 0, 600, 600)];
        ...
        if ([UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypePhotoLibrary]) {
            ...
            self.imgPicker.modalPresentationStyle = UIModalPresentationPopover; //调用本地相册，必须使用popover
            popPC = self.imgPicker.popoverPresentationController;
            popPC.permittedArrowDirections = UIPopoverArrowDirectionAny;
            popPC.delegate=self;
